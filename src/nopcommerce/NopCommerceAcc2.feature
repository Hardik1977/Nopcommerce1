Feature: Bookcategory
  ...
  Scenario Outline: I should be able to see list of terms of each filter
    ...
    Examples:
      | filter  | List                                                                         |
      | sort by | Name:A to Z, name:Z to A, Price: Low to High, Price:Hight to low, Created on |
      | Display | 3, 6, 9                                                                      |
  ...
  Scenario Outline: I should be able to see product display format according disply format type as per given picture in srs document
